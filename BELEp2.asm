...
checkNull:
	...
	add $a1, $zero, $zero   # zero out a1 so we can use it as "n"
		
getLen:
	lb  $t2, 0($a0)
	beq $t2, $t9, hashN
	
	addi $a0, $a0, 1	# update string index
	addi $a1, $a1, 1 	# increase len of string
	
	j getLen
			
hashN:				# assumes $a1 (str len) has been set
	lb   $t2, -1($a0)  	# t2 will hold the hex val of char at each interation
				# starts -1 index back because we read an extra index
				# for the new line character in getLen
	beqz $a1, printCalc	# goto end if we run out of chars
	...
	addi $a0, $a0, -1	# dec string address to read backwards
	addi $a1, $a1, -1       # subtract from loop counter
	j hashN		
